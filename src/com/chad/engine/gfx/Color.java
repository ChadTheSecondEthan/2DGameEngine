...
public class Color {
    ...
    public static int add(int c, int v) {
        return Mathf.clamp(((c / 100) % 10) + ((v / 100) % 10), 0, 5) * 100 +
            Mathf.clamp(((c / 10) % 10) + ((v / 10) % 10), 0, 5) * 10 +
            Mathf.clamp((c % 10) + (v % 10), 0, 5);
    }
    ...
    public static int randomRGB() { return randomRGB(0, 255); }
