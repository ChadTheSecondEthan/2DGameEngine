...
public class Font {
    ...
    public static int offset(char c) {
        for (int i = 0; i < LAYOUT.length; i++) {
            ...
            if ((j = LAYOUT[i].indexOf(c)) != -1) {
                return ((BASE_OFFSET_Y + i) * com.jdh.microcraft.gfx.Renderer.spritesheet.sizeSprites) + (BASE_OFFSET_X + j);
            }
        }

        // character not found -> ?
        return 15 * com.jdh.microcraft.gfx.Renderer.spritesheet.sizeSprites + 15;
    }
    ...
    public static void render(char c, int x, int y, int color, int backgroundColor) {
        ...
        if (backgroundColor != -1) {
            com.jdh.microcraft.gfx.Renderer.fill(x, y, 8, 8, backgroundColor);
        }

        com.jdh.microcraft.gfx.Renderer.render(
            Font.offset(c),
            x, y,
            Color.get(color, color, color, color),
            com.jdh.microcraft.gfx.Renderer.FLIP_NONE
        );
    }
